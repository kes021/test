...
#define AES_DEBUG_MODE

#ifdef AES_DEBUG_MODE
#include <stdio.h>
#include <stdarg.h>
...
static
void print_block_state (uint32_t *block, const char *prefix, ...)
{
    va_list arglist;
...
#else
#define print_block_state (void)sizeof
#endif
...
const uint8_t aes_sbox_inverse [256] = {
...
const uint32_t aes_tbox_inverse [256] =
{
...
static inline
uint32_t ror32 (uint32_t v, uint8_t s)
{
    if (s == 0)
        return v;
...
static inline
uint32_t sbox_substitute_word32 (uint32_t w, const uint8_t *sbox)
{
    return
        (sbox[(w >> 24) & 0xff] << 24) |
        (sbox[(w >> 16) & 0xff] << 16) |
...
{
    return
        rol32(tbox [(c >>  0) & 0xff],  0)    ^
        rol32(tbox [(c >>  8) & 0xff],  8)    ^
        rol32(tbox [(c >> 16) & 0xff], 16)    ^
        rol32(tbox [(c >> 24) & 0xff], 24);
}

/*
 *  AES-128 encryption/decryption subroutines
 */

static inline
void aes128_sub_bytes (uint32_t *result, uint32_t *state)
{
    result[0] = sbox_substitute_word32 (state[0], aes_sbox);
    result[1] = sbox_substitute_word32 (state[1], aes_sbox);
    result[2] = sbox_substitute_word32 (state[2], aes_sbox);
    result[3] = sbox_substitute_word32 (state[3], aes_sbox);
}

static inline
void aes128_sub_bytes_inverse (uint32_t *result, uint32_t *state)
{
    result[0] = sbox_substitute_word32 (state[0], aes_sbox_inverse);
    result[1] = sbox_substitute_word32 (state[1], aes_sbox_inverse);
    result[2] = sbox_substitute_word32 (state[2], aes_sbox_inverse);
    result[3] = sbox_substitute_word32 (state[3], aes_sbox_inverse);
}
...
static inline
void aes128_mix_columns_inverse (uint32_t *state)
{
    state[0] = tbox_mix_columns (state[0], aes_tbox_inverse);
    state[1] = tbox_mix_columns (state[1], aes_tbox_inverse);
    state[2] = tbox_mix_columns (state[2], aes_tbox_inverse);
    state[3] = tbox_mix_columns (state[3], aes_tbox_inverse);
}

static inline
void aes128_shift_rows (uint32_t *result, uint32_t *state)
{
    uint32_t cache[4] = {
...
static inline
void aes128_shift_rows_inverse (uint32_t *result, uint32_t *state)
{
    uint32_t cache[4] = {
        state[0], state[1], state[2], state[3]
    };
...
static inline
void aes128_add_round_key (uint32_t *state, uint32_t *plain, uint32_t *w)
{
    state[0] = plain[0] ^ w[0];
    state[1] = plain[1] ^ w[1];
    state[2] = plain[2] ^ w[2];
...
void aes128_block_encrypt (uint32_t *plain, uint32_t *w, uint32_t *chiff)
{
    ...
    print_block_state (plain, "round[ 0].input:\t");
    print_block_state (w, "round[ 0].k_sch:\t");


    aes128_add_round_key (state, plain, w);

    for (int i = 1; i < 10; i++)
    {
        print_block_state (state, "round[%2d].start:\t", i);

        aes128_sub_bytes (state, state);
        print_block_state (state, "round[%2d].s_box:\t", i);

        aes128_shift_rows (state, state);
        print_block_state (state, "round[%2d].s_row:\t", i);

        aes128_mix_columns (state);
        print_block_state (state, "round[%2d].m_col:\t", i);
        print_block_state (w + 4*i, "round[%2d].k_sch:\t", i);

        aes128_add_round_key (state, state, w + 4*i);
    }

    print_block_state (state, "round[10].start:\t");

    aes128_sub_bytes (state, state);
    print_block_state (state, "round[10].s_box:\t");

    aes128_shift_rows (state, state);
    print_block_state (state, "round[10].s_row:\t");

    print_block_state (w + 40, "round[10].k_sch:\t");

    aes128_add_round_key (chiff, state, w + 40);

    print_block_state (chiff, "round[10].output:\t");
}
...
void aes128_block_decrypt (uint32_t *chiff, uint32_t *w, uint32_t *plain)
{
    ...
    print_block_state (chiff, "round[ 0].iinput:\t");
    print_block_state (w + 40, "round[ 0].ik_sch:\t");

    aes128_add_round_key (state, chiff, w + 40);

    for (int i = 9; i > 0; i--)
    {
        print_block_state (state, "round[%2d].istart:\t", 10 - i);

        aes128_shift_rows_inverse (state, state);
        print_block_state (state, "round[%2d].is_box:\t", 10 - i);

        aes128_sub_bytes_inverse (state, state);
        print_block_state (state, "round[%2d].is_box:\t", 10 - i);

        print_block_state (w + 4*i, "round[%2d].ik_sch:\t", 10 - i);

        aes128_add_round_key (state, state, w + 4*i);
        print_block_state (state, "round[%2d].ik_add:\t", 10 - i);

        aes128_mix_columns_inverse (state);
    }

    print_block_state (state, "round[10].istart:\t");

    aes128_shift_rows_inverse (state, state);
    print_block_state (state, "round[10].is_box:\t");


    aes128_sub_bytes_inverse (state, state);
    print_block_state (state, "round[10].is_box:\t");

    print_block_state (w, "round[10].ik_sch:\t");
    aes128_add_round_key (plain, state, w);
    print_block_state (plain, "round[10].ioutput:\t");
}
...
{
    return sbox_substitute_word32 (w, aes_sbox);
}
...
void aes128_key_expand (uint32_t *key, uint32_t *w)
{
    ...
    while (i < 4 * (10 + 1))
    {
        ...
        if (i % 4 == 0)
        {
            temp = aes_sub_word (ror32 (temp, 8)) ^ Rcon [i / 4];
        }
...
/*
void generate_sbox (uint8_t sbox[256], uint8_t sbox_inv[256])
{
    for (uint32_t k = 0; k < 256; k++)
    {
        uint8_t inv = gf256inv (k), v = 0;

        for (uint32_t j = 0; j < 8; j++)
        {
            v |= ((__builtin_parity (inv & 0xF1)) << j);
            inv = (inv >> 1) | (inv << 7);
        }
    ...
    printf ("const uint32_t aes_tbox [256] = \n{\n");
