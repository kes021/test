...
void aes128_block_decrypt (uint32_t *chiff, uint32_t *w, uint32_t *plain)
{
    ...
    for (int i = 9; i > 0; i--)
    {
        printf ("round[%2d].istart:\t", 10 - i);
        print_block (state);

        aes128_shift_rows_inverse (state, state);
        printf ("round[%2d].is_box:\t", 10 - i);
        print_block (state);
        ...
        printf ("round[%2d].ik_sch:\t", 10 - i);
        print_block (w + 4*i);

        aes128_add_round_key (state, state, w + 4*i);
        printf ("round[%2d].ik_add:\t", 10 - i);
        print_block (state);

        aes128_mix_columns_inverse (state);
    }

    printf ("round[10].istart:\t");
    print_block (state);

    aes128_shift_rows_inverse (state, state);
    printf ("round[10].is_box:\t");
    print_block (state);
